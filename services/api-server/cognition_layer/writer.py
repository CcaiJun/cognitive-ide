...
# Custom YAML representer: serialize Enum as plain string value
def _enum_representer(dumper, data):
    return dumper.represent_str(data.value)

# Register for all Enum subclasses in our schemas
import schemas.cognition as _sc
for _name in dir(_sc):
    _obj = getattr(_sc, _name)
    if isinstance(_obj, type) and issubclass(_obj, Enum) and _obj is not Enum:
        yaml.add_representer(_obj, _enum_representer)
...
class CognitionWriter:
    ...
    def write_symbol_index(self, index: SymbolIndexSchema) -> Path:
        ...
        data = {
            "symbols": [
                {"name": s.name, "type": s.type, "node_id": s.node_id, "file": s.file, "line": s.line}
    ...
    def write_snapshot(self, snapshot: CognitionSnapshotSchema) -> Path:
        ...
        data = {
            ...
            "changes": [c.model_dump() if hasattr(c, 'model_dump') else c for c in snapshot.changes],
    ...
    def _cognition_node_to_dict(self, node: CognitionNodeSchema) -> Dict:
        data = {
            ...
            "responsibility": {"summary": node.responsibility.summary, "detail": node.responsibility.detail},
            "interfaces": [
                {
                    "name": i.name, "signature": i.signature, "access": i.access,
                    "side_effects": i.side_effects,
                    "callers": [{"node_id": c.node_id, "file": c.file} for c in i.callers],
                    "preconditions": i.preconditions, "complexity": i.complexity,
                }
                for i in node.interfaces
            ],
            "key_types": [{"name": k.name, "definition_in": k.definition_in, "fields": k.fields} for k in node.key_types],
            "dependencies": [
                {"node_id": d.node_id, "interface_file": d.interface_file, "used_for": d.used_for, "required": d.required}
                for d in node.dependencies
            ],
            "decisions": [
                {"id": dec.id, "title": dec.title, "context": dec.context, "decision": dec.decision, "consequences": dec.consequences, "linked_issue": dec.linked_issue}
                for dec in node.decisions
            ],
            "risks": [
                {"id": r.id, "level": r.level, "title": r.title, "description": r.description, "mitigation": r.mitigation, "status": r.status}
                for r in node.risks
            ],
        }
        if node.testing:
            data["testing"] = {"coverage_percent": node.testing.coverage_percent, "test_files": node.testing.test_files, "uncovered_cases": node.testing.uncovered_cases}
        if node.performance:
            data["performance"] = {"average_latency_ms": node.performance.average_latency_ms, "p99_latency_ms": node.performance.p99_latency_ms, "bottleneck": node.performance.bottleneck}
        if node.data_flow:
            data["data_flow"] = {
                "description": node.data_flow.description,
                "steps": [{"action": s.action, "input": s.input, "output": s.output, "calls": s.calls, "writes_to": s.writes_to, "publishes": s.publishes} for s in node.data_flow.steps],
            }
